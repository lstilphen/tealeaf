...
helpers do
  ...
  def winner!(msg)
    ...
    @winner = "<strong>#{session[:player]} wins!</strong> #{msg} #{session[:player]} now has $#{session[:pot]}."
    @show_buttons = false
    @play_again = true
  end
  ...
  def loser!(msg)
    session[:pot] -= session[:bet].to_i
    @loser = "<strong>#{session[:player]} loses!</strong> #{msg} #{session[:player]} now has $#{session[:pot]}."
    @show_buttons = false
    @play_again = true
  end
